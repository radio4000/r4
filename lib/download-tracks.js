...
const downloadTracks = async (tracks, downloadFolderName, options = {}) => {
	const {forceDownload, forceMediaAvailableCheck} = options
	...
	for (let index = 0; index < tracks.length; index++) {
		...
		const shouldDownload = () => {
			...
			if (forceMediaAvailableCheck && !fileExists) {
				console.log('Media available check forced', newTrackFilePath)
				return true
			}
			if (mediaNotAvailable) {
				console.log('Media available, skiping download', newTrackFilePath)
				return false
			}
			if (fileExists) {
				console.log('Track exists, not downloading', newTrackFilePath)
				return false
			}
			return false
		}
